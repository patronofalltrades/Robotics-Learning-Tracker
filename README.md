...
This is a private notebook, not a compliance system. Review Firebase region, retention, access, Google account, and Vercel preview settings for your own privacy requirements. The app does not use admin APIs, service-account credentials, or persistent offline caching: if you reload before retrying an unsaved write, that local draft is lost.
