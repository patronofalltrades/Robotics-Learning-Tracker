...
describe.skipIf(!enabled)("Firestore emulator owner isolation", () => {
  ...
  it("allows only the owner to access nested quiz attempts", async () => { const owner = env.authenticatedContext("a").firestore(); const other = env.authenticatedContext("b").firestore(); const attempt = doc(owner, "users", "a", "weeks", "week-1", "quizAttempts", "attempt-1"); await assertSucceeds(setDoc(attempt, { weekId: "week-1", questionId: "w1-q1" })); await assertSucceeds(getDoc(attempt)); await assertFails(getDoc(doc(other, "users", "a", "weeks", "week-1", "quizAttempts", "attempt-1"))); await assertFails(setDoc(doc(other, "users", "a", "weeks", "week-1", "quizAttempts", "attempt-2"), { bad: true })); await assertFails(getDoc(doc(env.unauthenticatedContext().firestore(), "users", "a", "weeks", "week-1", "quizAttempts", "attempt-1"))); });
