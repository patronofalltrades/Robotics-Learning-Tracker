...
export const emptyProgress = (weekId: string): WeekProgress => ({
  ...
  notes: { ...emptyNotes }, confidence: null, minutes: null, biggestQuestion: "", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
});
...
export function validateUserSettings(value: UserSettings): string[] {
  const errors: string[] = []; if (value.firstFriday && !isFridayDate(value.firstFriday)) errors.push("First Friday must be a valid Friday date."); if (!isValidTimezone(value.timezone)) errors.push("Choose a valid IANA timezone."); return errors;
}
export function normalizeSettings(value: Partial<UserSettings> | null | undefined): UserSettings {
  const candidate: UserSettings = { firstFriday: value?.firstFriday ?? null, timezone: value?.timezone ?? "America/Los_Angeles", onboardingStatus: value?.onboardingStatus === "complete" ? "complete" : "not_started", displayName: value?.displayName, email: value?.email, createdAt: value?.createdAt, updatedAt: value?.updatedAt };
  return validateUserSettings(candidate).length ? { ...candidate, firstFriday: null, timezone: "America/Los_Angeles", onboardingStatus: "not_started" } : candidate;
}
